...
class Tank_auth
{
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login))) {	// login ok
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						$this->error = array('banned' => TRUE);

					} else {
						$this->ci->session->set_userdata(array(
								'user_id'	=> $user->id,
								'username'	=> $user->username."*".$user->first_name."*".$user->last_name."*".$user->last_login,
                                'role'  	=> $user->role,
	...
	function is_mod()
	{
		return $this->ci->session->userdata('role') == 2;
	}
    /**
	 *Check user role if store owner
	 *
	 * @param	bool
	 * @return	bool
	 */
	...
	{
		return $this->ci->session->userdata('role') == 3;
	}
     /**
	 * Store owner only access their store 
	 *
	 * 
	 * @return	bool
	 */
	function is_owner($store_id,$owner_id)
	{
		return $this->ci->users->is_store_belongs_to_owner($store_id,$owner_id);
	}
	/**
	 * Get user_id
	 *
	...
	private function clear_login_attempts($login)
	{
    	...
    	/**
	 * Update user on the site and return some data about it:
	 * user_id, username, password, email, new_email_key (if any).
	 *
     * @param	int
	 * @param	string
	 ...
	 * @return	array
	 */
	function update_acc_details($uid,$username, $email, $banned, $email_activation,$role=2)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available2($username,$uid)) {
			$this->error = array('username' => 'auth_username_in_use');

		} elseif (!$this->ci->users->is_email_available2($email,$uid)) {
			$this->error = array('email' => 'auth_email_in_use');
            ...
            //if($banned==1)
            //$data["ban_reason"] = 'Your account has been disabled by the Administrator.';
//            else
//            $data["ban_reason"]=NULL;
           if ($email_activation) {
				$data['new_email_key'] = md5(rand().microtime());
			}
			if (!is_null($res = $this->ci->users->update_user_acc($uid,$data))) {
				$data['user_id'] = $uid;
			    unset($data['last_ip']);
				return $data;
			}
	 ...
	 * @return	array
	 */
	function update_password($uid, $password,$email)
	{
			// Hash password using phpass
			$hasher = new PasswordHash(
					$this->ci->config->item('phpass_hash_strength', 'tank_auth'),
			...
			if (!is_null($res = $this->ci->users->change_password($uid,$hashed_password))) {
				$data['user_id'] = $uid;
                $data['email']=$email;
				$data['password'] = $password;
				return $data;
    ...
    /**
     * Seo Friendly function for unique url
     * @param string(name)
     * @return string(unqiue_url)
     * */
     ...
     * @param topic id 
     * @param user id(logged in user)
     * @param table name
     * */
     function delete_common($id,$table_name,$pk_column_name)
     {
        ...
        $user_id = $this->ci->session->userdata('user_id');
			...
			if ($user_id) {			// success
				...
				$this->ci->users->delete_common($id,$user_id,$table_name,$pk_column_name);
