...
class Comments extends CI_Controller{
    ...
    public function get_comment_list()
    {
        ...
        $data = array('video_comments.is_deleted' =>'0');
        
        $this->datatables->select('videos.video_title , video_comments.body ,users.email, video_comments.created , video_comments.comment_id , video_comments.status')
                         ->join('users', 'users.id = video_comments.user_id', 'LEFT')
                         ->join('videos', 'videos.video_id = video_comments.video_id', 'LEFT')
                         ->where($data)
                         ->from('video_comments');
